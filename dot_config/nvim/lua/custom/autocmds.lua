...
local function is_writable_path(path)
  if path == "" then
    return false
  end
  ...
  -- If the file does not exist yet, allow saving if the parent directory is writable.
...
local function can_save(bufnr)
  ...
  return bo.buftype == ""        -- normal file buffer, not quickfix/terminal/help/etc.
    and bo.modifiable
    and not bo.readonly
    and bo.modified
    and name ~= ""               -- buffer must have a filename
    and is_writable_path(name)
end
...
vim.api.nvim_create_autocmd({
  "TextChanged",   -- Normal-mode changes
  "InsertLeave",   -- Leaving Insert mode
  "BufLeave",      -- Switching buffers
  "WinLeave",      -- Switching windows
  "FocusLost",     -- Terminal/app focus lost, if supported
}, {
  group = autosave_group,
  desc = "Autosave normal file buffers after edits",
